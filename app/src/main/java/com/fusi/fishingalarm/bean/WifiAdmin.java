package com.fusi.fishingalarm.bean;
...
public class WifiAdmin {
    ...
    public String getBroadcastIp() {
        int gatewayVal = wifiManager.getDhcpInfo().gateway;
    ...
    public void clearWifiConfig(Context context) {
        List<WifiConfiguration> wifiConfigList = wifiManager.getConfiguredNetworks();// 得到配置好的网络信息
        int nowid = wifiManager.getConnectionInfo().getNetworkId();
        for (int i = 0; i < wifiConfigList.size(); i++) {
            WifiConfiguration wif = wifiConfigList.get(i);
            int sid = wif.networkId;
            if (sid != nowid) {    // 清楚该Wifi之外的所有配置信息
                wifiManager.removeNetwork(wif.networkId);
            }
        }
        wifiManager.saveConfiguration();
    }

    public void clearWrongInputWifiConfig(Context context) {
        List<WifiConfiguration> wifiConfigList = wifiManager.getConfiguredNetworks();// 得到配置好的网络信息
        int nowid = wifiManager.getConnectionInfo().getNetworkId();
        for (int i = 0; i < wifiConfigList.size(); i++) {
            WifiConfiguration wif = wifiConfigList.get(i);
            Log.i(TAG, "clearWrongInputWifiConfig: wif.SSID:"+wif.SSID);
            int sid = wif.networkId;
            if (sid == nowid) {    // 清楚该Wifi之外的所有配置信息
                wifiManager.removeNetwork(wif.networkId);
            }
    ...
    public void clearWrongKnowSSidWifiConfig(Context context,String ssid) {
        List<WifiConfiguration> wifiConfigList = wifiManager.getConfiguredNetworks();// 得到配置好的网络信息
//        int nowid = wifiManager.getConnectionInfo().getNetworkId();
        for (int i = 0; i < wifiConfigList.size(); i++) {
            WifiConfiguration wif = wifiConfigList.get(i);
            Log.i(TAG, "clearWrongInputWifiConfig: wif.SSID:"+wif.SSID+"ssid:"+ssid);
//            int sid = wif.networkId;
            if (ssid.equals(wif.SSID)) {    // 清楚该Wifi之外的所有配置信息
                wifiManager.removeNetwork(wif.networkId);
            }
        }
        wifiManager.saveConfiguration();
    ...
    public boolean connectWiFI(String SSID,String pwd) {
        ...
        config = CreateWifiInfo(SSID, pwd, WifiCipherType.WIFICIPHER_WPA);
